...
std::pair<bool, WinTimeZoneRegistryEntry> ReadTimeZoneInfo(
  ...
  // Real-world offsets fit within +/-14 hours; accept anything up to a week as
  // an arbitrary-but-generous bound. It exists so that downstream arithmetic
  // such as -60 * (Bias + DaylightBias) in the transition-table builder stays
  // far from std::int_fast32_t overflow even for corrupt registry data.
...
std::wstring Utf8ToUtf16(const std::string& utf8str) {
  ...
  const int required = ::MultiByteToWideChar(
      CP_UTF8, MB_ERR_INVALID_CHARS, utf8str_ptr, utf8str_len, nullptr, 0);
  if (required <= 0) {
    return std::wstring();
  }
...
WinTimeZoneRegistryInfo LoadWinTimeZoneRegistry(const std::string& name) {
  const std::wstring key_name = ConvertToWindowsTimeZoneId(Utf8ToUtf16(name));
  ...
  if (hkey_dynamic_years) {
    ...
    // Require the same year range as SYSTEMTIME (see ToWinSystemTime). This
    // also bounds the year range that the transition-table builder expands
    // entry-per-year, so corrupt or malicious registry data (e.g.
    // FirstEntry == LastEntry == 4000000000) cannot make it loop over an absurd
    // number of years.
